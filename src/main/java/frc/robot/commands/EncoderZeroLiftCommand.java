...
public class EncoderZeroLiftCommand extends Command{
    ...
    public EncoderZeroLiftCommand(LiftSubsystem subsystem){
        this.lift = subsystem;
        lift.refresh();
    }
    ...
    public void execute(){
        switch(state){
            ...
            case WAIT_FOR_SPINUP:
                ...
                break;
            case TRAVEL:
                if(lift.motor.getEncoder().getVelocity() > -10.0){
                    lift.getEncoder().setPosition(-10);
                    isFinished = true;
                    if(Constants.DEBUG_MODE) System.out.println("Reached bottom of lift for the " + (lift.motor.getDeviceId() == 12? "right": "left") + " lift");
                }
                break;        
        }
    }
